...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    if (solver_flag == INITIAL)//进行初始化
    {
        ...
        if (frame_count == WINDOW_SIZE)
        {
            ...
            frame_count++; // 滑动窗口没塞满，接着塞,图像帧数量+1
    }
    else//紧耦合的非线性优化
    {
        TicToc t_solve;
 ...
 * @param[out]   L 保存滑动窗口中与当前帧满足初始化条件的那一帧（第一帧可能是）
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        {
            //计算平均视差
            double sum_parallax = 0;//parallax视差
...
{
    // 窗口第一帧之前的位姿
    Vector3d origin_R0 = Utility::R2ypr(Rs[0]);//得到row pitch yaw的旋转角度
    ...
    //TODO
    //rot_diff应该是滑动窗口`第一帧优化前后位姿差相对于世界系的旋转
    Matrix3d rot_diff = Utility::ypr2R(Vector3d(y_diff, 0, 0));//724 726 这里应该特殊处理，得到位姿旋转矩阵，我记得我在深蓝的ppt看过
...
bool Estimator::failureDetection()
{
    ...
    */
    ...
    Matrix3d delta_R = tmp_R.transpose() * last_R;//转置
...
void Estimator::optimization()
{
    ...
    //添加边缘化残差
    //添加marginalization的residual，这个marginalization的结构是始终存在的，随着下面marginazation的结构更新，
    //last_marginalization_parameter_blocks对应的是还在sliding window的变量,关于这一部分的理解，
    //请看heyijia大神的BLOG:http://blog.csdn.net/heyijia0327/article/details/53707261, 可能是说的最清楚的一个了
    //这里可以这样理解，下面会添加对IMU和视觉的残差，但是，这些对应的变量实际上跟之前被margin掉的变量是有约束的，
    //这里的last_marginalization_parameter_blocks就是保存的这些变量，也就是heyijia博客中对应的Xb变量，
    ...
    //添加IMU残差
     //这里IMU项和camera项之间是有一个系数，这个系数就是他们各自的协方差矩阵：IMU的协方差是预积分的协方差(IMUFactor::Evaluate，
     //中添加IMU协方差，求解jacobian矩阵)，而camera的测量残差则是一个固定的系数（f/1.5）
    //添加IMU的residual
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        int j = i + 1;
    ...
    //parameter_block_data parameter_block_idx parameter_block_size是marinazation中存参数块的容器(unordered_map),
    //key都是addr,分别对应这些参数的data，在稀疏矩阵A中的index(要被margin掉的参数会被移到前面)，A中的大小
...
//则把这帧的视觉测量舍弃掉而保留IMU测量值在滑动窗口中这样的策略会保证系统的稀疏性
void Estimator::slideWindow()
{
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        if (frame_count == WINDOW_SIZE)
        {
           ...
           // 滑动窗口中最新帧的时间戳、位姿和bias，用滑动窗口中第2最新帧的数据来初始化
            ...
            if (true || solver_flag == INITIAL)//为啥是ture 这不是相当于一定执行吗
            {
    ...
    else
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            {
                double tmp_dt = dt_buf[frame_count][i];// dt_buf[frame_count][i]可以这么用吗，涉及知识盲区
...
void Estimator::slideWindowOld()
{
    ...
    if (shift_depth)
    {
        ...
        R0 = back_R0 * ric[0];//back_R0 应该是被边缘化最老帧在世界坐标系的旋转
        R1 = Rs[0] * ric[0];
        P0 = back_P0 + back_R0 * tic[0];//back_P0 应该是被边缘化最老帧在世界坐标系的位置
        P1 = Ps[0] + Rs[0] * tic[0];
