...
class FeaturePerId
{
  public:
    ...
    int start_frame;
    vector<FeaturePerFrame> feature_per_frame;//观测到该角点的每个图像帧

    int used_num;//角点在图像出现过的数量
    bool is_outlier;
    bool is_margin;
...
class FeatureManager
{
  public:
    ...
    int last_track_num;// 最新帧图像被跟踪到的特征点的数量

  private:
    double compensatedParallax2(const FeaturePerId &it_per_id, int frame_count);
    const Matrix3d *Rs;
    Matrix3d ric[NUM_OF_CAM];//应该是相机到imu的外参 即旋转
};
