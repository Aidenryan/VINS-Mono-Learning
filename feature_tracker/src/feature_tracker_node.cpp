...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    // 并不是每读入一帧图像，就要发布特征点
    // 判断间隔时间内的发布次数
    if (round(1.0 * pub_count / (img_msg->header.stamp.toSec() - first_image_time)) <= FREQ)//FREQ=100 maybe
    {
        PUB_THIS_FRAME = true;
...
//ROS图像和OoenCV转换联系的一个类
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)// NUM_OF_CAM相机个数
    {
   ...
   if (PUB_THIS_FRAME)
   {
        ...
        // skip the first image; since no optical（光流） speed on frist image
        if (!init_pub)//第一帧不发布
        {
            init_pub = 1;
        }
...
int main(int argc, char **argv)
{
    ...
    //读取每个相机实例对应的相机内参,在这里NUM_OF_CAM设置成常量1，只有一个摄像头（单目），读取相机内参
    for (int i = 0; i < NUM_OF_CAM; i++) 
